...
import { Search, TrendingUp, Hash, Image as ImageIcon, Radio, CalendarDays, Bookmark, List, ArrowRight } from 'lucide-react';
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Link to="/live">
          <Card className="p-6 hover:shadow-lg transition-shadow cursor-pointer group">
            <div className="size-12 rounded-full bg-red-500/10 flex items-center justify-center mb-3 group-hover:bg-red-500/20 transition-colors">
              <Radio className="size-6 text-red-600" />
            </div>
            <h3 className="font-semibold mb-1">Live Streams</h3>
            <p className="text-sm text-muted-foreground">Watch live content</p>
          </Card>
        </Link>
        <Link to="/events">
          <Card className="p-6 hover:shadow-lg transition-shadow cursor-pointer group">
            <div className="size-12 rounded-full bg-blue-500/10 flex items-center justify-center mb-3 group-hover:bg-blue-500/20 transition-colors">
        ...
        <Link to="/collections">
          <Card className="p-6 hover:shadow-lg transition-shadow cursor-pointer group">
            <div className="size-12 rounded-full bg-purple-500/10 flex items-center justify-center mb-3 group-hover:bg-purple-500/20 transition-colors">
              <Bookmark className="size-6 text-purple-600" />
            </div>
            <h3 className="font-semibold mb-1">Collections</h3>
            <p className="text-sm text-muted-foreground">Organize saved posts</p>
          </Card>
        </Link>
        <Link to="/lists">
          <Card className="p-6 hover:shadow-lg transition-shadow cursor-pointer group">
            <div className="size-12 rounded-full bg-green-500/10 flex items-center justify-center mb-3 group-hover:bg-green-500/20 transition-colors">
              <List className="size-6 text-green-600" />
            </div>
            <h3 className="font-semibold mb-1">Lists</h3>
            <p className="text-sm text-muted-foreground">Curate user groups</p>
          </Card>
        </Link>
      </div>
        ...
        <TabsContent value="trending" className="mt-6 space-y-4">
          ...
          <div className="grid gap-3">
            {trendingHashtags.map((item, index) => (
              <Link key={index} to={`/hashtag/${item.tag}`}>
                <Card
